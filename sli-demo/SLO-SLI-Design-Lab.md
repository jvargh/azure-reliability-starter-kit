...
How this fits with the other document in `sli-demo/`:
...
`load/generate-traffic-all.ps1`) so the metrics exist.
...
so every evaluation window has samples (the demo uses `load/generate-traffic-all.ps1`).
