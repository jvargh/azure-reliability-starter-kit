# teardown.ps1
# Reverses deploy.ps1: deletes the health model (which removes its authentication settings and
# discovery rules), removes the Monitoring Reader role assignment on the SLI resource group, and
# optionally deletes the resource group. The SLI demo (../sli-demo) is left untouched.
#
# Usage:
#   ./teardown.ps1
